...
from utils.yf_session import shared_session
...
def _ticker(sym: str) -> yf.Ticker:
    # yfinance supports a custom requests session
    return yf.Ticker(sym, session=shared_session)


def _history(sym: str, period: str, interval: str) -> pd.DataFrame:
    """
    Try Ticker().history first. If empty, fallback to yf.download.
    """
    try:
        t = _ticker(sym)
    ...
    try:
        dl = yf.download(
            tickers=sym, period=period, interval=interval, progress=False, session=shared_session
        )
        if dl is not None and not dl.empty:
            if isinstance(dl.columns, pd.MultiIndex):
                # pick single symbol columns
                dl = dl.xs(sym, axis=1, level=1, drop_level=True)
            dl = dl.rename(columns={c: c.title() for c in dl.columns})
            return dl.reset_index().rename(columns={"Date": "date"})
    except Exception:
        pass
...
def fetch_quotes(symbols: List[str], chunk_size: int = 20, sleep_ms: int = 300) -> list[dict]:
    """
    Fetch 'live-ish' quotes for many symbols with a shared session and backoff.
    """
    out: list[dict] = []
    now = int(time.time())
    ...
    for batch in _chunks(symbols, chunk_size):
        for sym in batch:
            ...
            for c in candidates:
                try:
                    ...
                    if price is None:
                        # last resort — download last close
                        h2 = yf.download(tickers=c, period="5d", interval="1d", progress=False, session=shared_session)
                        if h2 is not None and not h2.empty:
                            if isinstance(h2.columns, pd.MultiIndex):
                                h2 = h2.xs(c, axis=1, level=1, drop_level=True)
                            price = float(h2["Close"].iloc[-1])
                            prev = float(h2["Close"].iloc[-2]) if len(h2) > 1 else None
        ...
        # increased delay to reduce throttling
